...
<html>
	...
	<body>
		...
		<div class="mui-content">
		    <ul class="mui-table-view">
	            <li class="mui-table-view-cell head-box" style="line-height: 40px;" onclick="changeImg()">
	               	头像
	               	<img src="${user.img }" width="40" height="40" class="right" id="img" />
	            </li>
	            ...
	            <li id="jg" class="mui-table-view-cell">
                	籍贯
                	<span>${teacher.jg }</span>
	            </li>
	            <li id="birth" class="mui-table-view-cell">
                	出生日期
                	<input type="text"  placeholder="请选择出生年月" data-options='{"type":"date"}' name="csrq" id="birthday" value="${teacher.csrq }" readOnly/>
	            ...
	            <li class="mui-table-view-cell">
                	 <a class="mui-navigate-right" href="<%=basePath%>Account/getbangclass.html">
						绑定关联班级	
                    </a>
	            </li>
	        </ul>
		...
		<nav class="mui-bar mui-bar-tab">
			<a id="defaultTab" class="mui-tab-item1" href="<%=basePath%>Account/getindextea.html">
			...
			<a class="mui-tab-item1" href="<%=basePath%>Account/getsettea.html">
	...
	<script type="text/javascript">
		document.getElementById("nickname").addEventListener('tap', function(e) {
				...
				var btnArray = ['取消', '确定'];
				mui.prompt('', '${teacher.nickname}', '修改昵称', btnArray, function(e) {
					if (e.index == 1) {
			...
			document.getElementById("name").addEventListener('tap', function(e) {
				e.detail.gesture.preventDefault(); //修复iOS 8.x平台存在的bug，使用plus.nativeUI.prompt会造成输入法闪一下又没了
				var btnArray = ['取消', '确定'];
				mui.prompt('', '${teacher.name}', '更改姓名', btnArray, function(e) {
					if (e.index == 1) {
			...
			document.getElementById("jg").addEventListener('tap', function(e) {
				e.detail.gesture.preventDefault(); //修复iOS 8.x平台存在的bug，使用plus.nativeUI.prompt会造成输入法闪一下又没了
				var btnArray = ['取消', '确定'];
				mui.prompt('', '${teacher.jg}', '更改籍贯', btnArray, function(e) {
					if (e.index == 1) {
						if(!e.value.match(/^[\u4e00-\u9fa5]+$/)){
							var btnArray = ['确定'];
							mui.alert('籍贯只能为中文!', '系统提示', btnArray, function(e) {
							});
							return false;
						}
						if(e.value.length < 2 || e.value.length > 10){
							var btnArray = ['确定'];
							mui.alert('籍贯只能在2-10位字符!', '系统提示', btnArray, function(e) {
							});
							return false;
						}
						if(1 < e.value.length <= 10){
							$.post('<%=basePath%>Account/updateUserInfo.html',{key:'jg',val:e.value,type:'String'},function(){
								window.location.reload();
							});
						}
					} else {
			...
			document.getElementById("qq").addEventListener('tap', function(e) {
				e.detail.gesture.preventDefault(); //修复iOS 8.x平台存在的bug，使用plus.nativeUI.prompt会造成输入法闪一下又没了
				var btnArray = ['取消', '确定'];
				mui.prompt('', '${teacher.qq}', '更改QQ', btnArray, function(e) {
					if (e.index == 1) {
			...
			document.getElementById("email").addEventListener('tap', function(e) {
				e.detail.gesture.preventDefault(); //修复iOS 8.x平台存在的bug，使用plus.nativeUI.prompt会造成输入法闪一下又没了
				var btnArray = ['取消', '确定'];
				mui.prompt('', '${teacher.email}', '更改邮箱', btnArray, function(e) {
					if (e.index == 1) {
						...
						$.post('<%=basePath%>Account/updateUserInfo.html',{key:'email',val:e.value,type:'String'},function(){
							window.location.reload();
						});
					} else {
						//取消
			...
			document.getElementById("phone").addEventListener('tap', function(e) {
				e.detail.gesture.preventDefault(); //修复iOS 8.x平台存在的bug，使用plus.nativeUI.prompt会造成输入法闪一下又没了
				var btnArray = ['取消', '确定'];
				mui.prompt('', '${teacher.phone}', '修改手机', btnArray, function(e) {
					if (e.index == 1) {
				...
				btns.each(function(i, btn) {
					btn.addEventListener('tap', function() {
						...
						picker.show(function(rs) {
							...
							$.post('<%=basePath%>Account/updateUserInfo.html',{key:'csrq',val:result.value,type:'String'},function(){
								window.location.reload();
							});
							picker.dispose();
						});
