...
<base href="<%=basePath%>">
	...
	$.ajax({
	url:"<%=basePath%>article/getActivityTeacher.html",
		type:"post",
		dataType:"json",
		async:false,
		success:function(list){
		for(var i=0;i<list.length;i++){
		$("#activity").append('<div class="activeBox"><img src="<%=basePath%>img/1.jpg" /><div class="activeInfo"><h4><a href="article/getActivityTeacherDetails.html?id='+list[i].id+'">'+list[i].title+'</a></h4><p>'+list[i].context+'</p><span class="logTime">'+list[i].subTime+'<span></div>')
		}
		}	
	});
	</script>
