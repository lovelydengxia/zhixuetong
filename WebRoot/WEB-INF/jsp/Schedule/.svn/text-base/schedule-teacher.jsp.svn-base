...
#sel_class{background-color: transparent; font-size: 26px; min-width: 200px;height: 30px;border: 1px solid #ccc !important; margin:10px 80px; padding: 0;}
</style>
</head>
...
<body>
	...
	<a class="mui-action-back mui-icon mui-icon-left-nav mui-pull-left" href="javascript:void(0)">
				...
				for(var j=1;j<=5;j++){
					...
					$.each(d,function(n,item){
						if(item.weekID==j&&item.lessonID==i){
							$("#"+i+"_"+j).text(item.subject+" "+item.className);
						}
					})
				}
			}
...
<script type="text/javascript">
	$(function(){
		var id=$("#hide_id").val();
		$.ajax({
			url:"<%=basePath%>schedule/getClassByTeacherID.html",
			type:"POST",
			data:{id:id},
			success:function(data){
				...
				$.each(list,function(i,item){
					op+="<option value='"+item.cid+"'>"+item.bjmc+"</option>";
				})
				$("#sel_class").html(op)
				$("#sel_class").on("change",function(){
					var cid=$("#sel_class").val();
					$.ajax({
