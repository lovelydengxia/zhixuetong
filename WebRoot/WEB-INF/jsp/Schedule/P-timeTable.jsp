...
<base href="<%=basePath%>">
	...
	<body>
		...
		<div >
			...
			<div style="margin-top: 55px;">
				<label style="padding-left: 10px; ">请选择学生:</label> 
					<select class="form-control" name="studentID" id="student-id-select" onChange="getID(this.value);">
					</select>	
			</div>
	...
	<script type="text/javascript">
		...
		function getID(studentID){
			var accountID = $("#accountID").val();
			$.ajax({
				url:"<%=basePath%>schedule/getScheduleByStudentIDAndAccountID.html",
				type:"POST",
				data:{accountID:accountID,
					studentID:studentID},
				dataType:"json",
							...
							for(var i = 1 ; i <= 10 ; i++){
								for(var x = 0 ; x < data.length ; x++){
									if(data[x].weekID == j && data[x].lessonID == i ){
										...
										$("#"+j+"_"+i+"_2").text(data[x].teacherName);
										break;
									}else{
										$("#"+j+"_"+i+"_1").text("");
										$("#"+j+"_"+i+"_2").text("");
