...
<body>
	...
	<div class="mui-content">
		<table class="table">
			<tr>
				<td><label>标题：</label>${work.hwtitle }</td>
				<td><label>日期：</label>${work.deadline }</td>
			</tr>
			<tr>
				<td><label>科目：</label>${work.subid }</td>
				<td><label>发布者：</label>${tea}</td>
			</tr>
			<tr>
				<td><label>班级：</label>${classInfo.bjmc}</td>
				<td><label>学生：</label>
				<a type="button" href="<%=basePath%>work/hwdetailstu.html?hwid=${work.id }"> 查看</a>
				</td>
			</tr>
		</table>
		<ul class="homework ">
			...
			<li>
				<a type="button" href="<%=basePath%>/work/deleteWork.html?id=${work.id}" class="mui-btn mui-btn-success mui-btn-outlined">
					撤销作业
				</a>
				<button type="button" id="gubook" class="right mui-btn mui-btn-warning ">
					留言
				</button>
				<span id="msg" style="color:red"></span>
			</li>
			<li class="mui-table-view-cell mesg hide">
					<form action="<%=basePath %>work/addGuest.html" method="post" onsubmit="return check()">
						<input type="hidden" name="hwid" value="${work.id }">
						<input type="text" name="content" style="margin-bottom: 1px;" id="content1">
						
						<div style="height: 40px;">
							<button type="submit" id="sub" class=" mui-btn mui-btn-warning mui-btn-sm right" >提交</button>
						</div>
					</form>
			</li>
		</ul>
		...
		<s:forEach items="${guestbooks }" var="gb">
				...
				<li class="mui-table-view-cell mui-media mesg2 hide">
					<form action="<%=basePath %>work/replay.html" method="post" class="aa" onsubmit="return check()">
						...
						<input type="text" name="content" style="margin-bottom: 1px;" id="content2">
						<button type="submit"  id="ss" class="mui-btn-sm mui-btn mui-btn-warning right" >提交</button>
					</form>
				</li>
			</ul>
